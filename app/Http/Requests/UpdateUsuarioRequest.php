...
class UpdateUsuarioRequest extends FormRequest
{
    ...
    /**
     * Reglas de validación para la actualización de usuarios
     * 
    ...
    public function rules(): array
    {
        ...
        // Si es un modelo User, obtener el ID
        if (is_object($usuarioId)) {
        ...
        $reglas = [
            'nombre_usuario' => [
                ...
                'string',
                'max:255',
                Rule::unique('usuarios', 'nombre_usuario')->ignore($usuarioId, 'id_usuario'),
            ],
            'nombres' => [
                'required',
                'string',
    ...
    public function messages(): array
    {
        return [
            ...
            'clave.required' => 'La contraseña es obligatoria si desea cambiarla.',
    ...
    protected function prepareForValidation(): void
    {
        ...
        // Si la contraseña está vacía, removerla para que no se valide
        if ($this->has('clave') && empty(trim($this->clave))) {
            $this->merge([
                'clave' => null,
